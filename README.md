...
- создаёт и отправляет задания в систему управления очередью Polus;
- собирает результаты вычислений и сохраняет их в solution/;
...
3. Гибридный режим MPI+OpenMP — распараллеливание внутренних циклов по узлам MPI-процессов.

Дополнительно реализовано:
1. визуализация решений (2D и 3D);
2. автоматическое разбиение области на поддомены с соблюдением геометрических пропорций;
3. сбор результатов от всех процессов и сохранение в CSV.


Выходные данные
- Численные решения для различных размеров сетки находятся в каталоге solution/.
- Визуализации в каталоге plot/ демонстрируют распределение потенциала (2D и 3D).
- В debug.log фиксируется поведение невязки (остатка) по итерациям метода PCG.
